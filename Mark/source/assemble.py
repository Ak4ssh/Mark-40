...
async def join(_, e: Message):
    ind = e.text[6:]
    chid = e.chat.id
    invitelink = chid.invite_link
    if invitelink is None:
        invitelink = (
        await MARK.export_chat_invite_link(
         chat_id
         )
        )
...
async def join(_, e: Message):
    ...
    try:
        if Session:
            await Session.leave_chat(invitelink)
            count += 1
        if Session2:
            await Session2.leave_chat(invitelink)
            count += 1
        if Session3:
            await Session3.leave_chat(invitelink)
            count += 1
        if Session4:
            await Session4.leave_chat(invitelink)
            count += 1
        if Session5:
            await Session5.leave_chat(invitelink)
            count += 1
        if Session6:
            await Session6.leave_chat(invitelink)
            count += 1
        if Session7:
            await Session7.leave_chat(invitelink)
            count += 1
        if Session8:
            await Session8.leave_chat(invitelink)
            count += 1
        if Session9:
            await Session9.leave_chat(invitelink)
            count += 1
        if Session10:
            await Session10.leave_chat(invitelink)
            count += 1
        if Session11:
            await Session11.leave_chat(invitelink)
            count += 1
        if Session12:
            await Session12.leave_chat(invitelink)
            count += 1
        if Session13:
            await Session13.leave_chat(invitelink)
            count += 1
        if Session14:
            await Session14.leave_chat(invitelink)
            count += 1
        if Session15:
            await Session15.leave_chat(invitelink)
            count += 1
        if Session16:
            await Session16.leave_chat(invitelink)
            count += 1
        if Session17:
            await Session17.leave_chat(invitelink)
            count += 1
        if Session18:
            await Session18.leave_chat(invitelink)
            count += 1
        if Session19:
            await Session19.leave_chat(invitelink)
            count += 1
        if Session20:
            await Session20.leave_chat(invitelink)
            count += 1
    except Exception as ex:
        await e.reply_text(f"ERROR")
